...
double z_production_rate(int x, int y, int z){
    return lambda_z*x;
}
...
double rate_total_function(int x, int y, int z, int w){
    return ( x_production_rate(x,y,z,w) + x_degradation_rate(x,y,z) + y_production_rate(x,y,z,w) + y_degradation_rate(x,y,z) + z_production_rate(x,y,z) + z_degradation_rate(x,y,z) + w_production_rate(x,y,z) + w_degradation_rate(x,y,z,w));
}
...
main()
{
    ...
    FILE *ff = fopen("with-causality-data-system-1.txt", "w"); //
