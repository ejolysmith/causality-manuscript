...
system_number = '4'  #first argument after .py file is number of cores
volume_number = '1'
...
def s5_v1_covariance(inputs2):
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = ( lamda * (z / volume) ) / (1 + ((x / volume)/k)**2 )
            x_death_rate = beta_x * x
            
            y_birth_rate = 2 * ( lamda * (z / volume) ) / (1 + ((x / volume)/k)**2 )
            y_death_rate = beta_y * y
            
            z_birth_rate = lamda_z
            z_death_rate = z * beta_z
            ...
            maximal_rate = 3 * lamda * z / V0   #Set with the assumption that volume at most
                                                                    #doubles (ensured by parameteres of cell divison)
            
            shadow_birth_rate = maximal_rate - (x_birth_rate + y_birth_rate )
