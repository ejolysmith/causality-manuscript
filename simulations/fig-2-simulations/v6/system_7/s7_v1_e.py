...
system_number = '7'  #first argument after .py file is number of cores
volume_number = '1'
...
def s5_v1_covariance(inputs2):
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = 2 * lamda * ( y/volume + 0.01 )  / ( y/volume + 0.01 + ( (x/volume) / (0.01 + w/volume))**2 )
            x_death_rate = beta_x * x
            
            y_birth_rate = lamda * ( y/volume + 0.01 ) / ( y/volume + 0.01 + ( (x/volume) / (0.01 + w/volume))**2 )
            y_death_rate = beta_y * y
            
            z_birth_rate = lamda_z * w /volume
            z_death_rate = beta_z * z **2 
            
            w_birth_rate = lamda_w * volume
            w_death_rate =  w * beta_w
            
            
            
            maximal_rate =  lamda_z * w /V0 + 2 * lamda  + lamda + lamda_w * Vmax  #Set with the assumption that volume at most
                                                                    #doubles (ensured by parameteres of cell divison)
            
            shadow_birth_rate = maximal_rate - (x_birth_rate + y_birth_rate + z_birth_rate + w_birth_rate)
