...
system_number = '10'  #first argument after .py file is number of cores
volume_number = '1'
...
#scratch_dir  = /scratch/smitheua/#os.environ['SCRATCH']
path_2 = '/scratch/smitheua/v6'
sys.path.append(path_2)
import new_vol_dyn
import new_vol_dyn_6
#import pandas as pd
...
jump_times = np.arange(1, jump_time_1_factor, 1)
...
def s5_v1_covariance(inputs2):
    ...
    volume = new_vol_dyn.exp_V(V0, interval_time)
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = A * (np.sin(omega * time) + 1) * volume
            x_death_rate = beta_x * x * z
            
            y_birth_rate = B * (np.sin(omega * time) + 1) * volume
            y_death_rate = beta_y * y * z
            
            z_birth_rate = C * (np.sin(omega * time) + 1) * volume
            ...
            maximal_rate = 2 * (A * Vmax + B * Vmax + C * Vmax)   #Set with the assumption that volume at most
                                                                    #doubles (ensured by parameteres of cell divison)
            
            shadow_birth_rate = maximal_rate - (x_birth_rate + y_birth_rate + z_birth_rate)
...
# =============================================================================
                ...
                temp_vol = new_vol_dyn.exp_V(V0 ,  interval_time) * stay_prob[T]
                V0 = temp_vol
                interval_time = 0
                volume = (new_vol_dyn.exp_V(V0, interval_time))
...
# =============================================================================
    ...
    while time < endtime:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            if time + t < jump_times[T]:
                ...
                temp_conc_x_average = x * new_vol_dyn.temp_exp_one_over_vol_average(V0, waittimes, interval_time) / waittimes
                ith_conc_x_avg = new_vol_dyn.running_abundance_avg(temp_conc_x_average, ith_conc_x_avg, waittimes, time)
                
                temp_conc_y_average = y * new_vol_dyn.temp_exp_one_over_vol_average(V0, waittimes, interval_time) / waittimes
                ith_conc_y_avg = new_vol_dyn.running_abundance_avg(temp_conc_y_average, ith_conc_y_avg, waittimes, time)
                
                temp_conc_z_average = z * new_vol_dyn.temp_exp_one_over_vol_average(V0, waittimes, interval_time)/ waittimes
                ith_conc_z_avg = new_vol_dyn.running_abundance_avg(temp_conc_z_average, ith_conc_z_avg, waittimes, time)
                
                temp_xz_average = x * z * new_vol_dyn.temp_exp_one_over_vol_sq_average(V0, waittimes, interval_time)/ waittimes
                ith_xz_v2_avg = new_vol_dyn.running_abundance_avg(temp_xz_average, ith_xz_v2_avg, waittimes, time)
                
                temp_yz_average = y * z * new_vol_dyn.temp_exp_one_over_vol_sq_average(V0, waittimes, interval_time)/ waittimes
                ith_yz_v2_avg = new_vol_dyn.running_abundance_avg(temp_yz_average, ith_yz_v2_avg, waittimes, time)
                
                temp_x_sq_average = x *x * new_vol_dyn.temp_exp_one_over_vol_sq_average(V0, waittimes, interval_time)/ waittimes
                ith_x_sq_v2_avg = new_vol_dyn.running_abundance_avg(temp_x_sq_average, ith_x_sq_v2_avg, waittimes, time)
                
                temp_y_sq_average = y *y * new_vol_dyn.temp_exp_one_over_vol_sq_average(V0, waittimes, interval_time)/ waittimes
                ith_y_sq_v2_avg = new_vol_dyn.running_abundance_avg(temp_y_sq_average, ith_y_sq_v2_avg, waittimes, time)
                
                temp_z_sq_average = z *z * new_vol_dyn.temp_exp_one_over_vol_sq_average(V0, waittimes, interval_time)/ waittimes
    ...
    return eta_xz_conc, eta_yz_conc, pearson_xz_conc, pearson_yz_conc ,eta_xz_numbers, eta_yz_numbers, pearson_xz_numbers, pearson_yz_numbers, lamda, lamda_z, lamda_w, beta_x, beta_y, beta_z, beta_w, k, A, B, C, omega
...
def parallel_s5_v1(inputs):
    ...
    products2 = [lamda2, lamda_z2, lamda_w2  ,  beta_x2 ,beta_y2, \
            beta_z2,beta_w2,  k2, V02,Vmax2,  a, 0,0, 1,  1, 1, 1, jump_time_1_factor, A2, B2, C2, omega2]
   ...
   # stay_prob[0: -1 : 2] = 0.6
    ...
    while (AA > 0):
        ...
        for _ in range(len(result)):
            ...
            #print(_)    
            eta_xz_conc, eta_yz_conc, pearson_xz_conc, pearson_yz_conc, eta_xz_numbers, eta_yz_numbers, pearson_xz_numbers, pearson_yz_numbers, lamda, lamda_z, lamda_w, beta_x, beta_y, beta_z, beta_w, k, A, B, C, omega = result[_]  #s5_v1_covariance(lamda, lamda_z, lamda_w  ,  beta_x ,beta_y, \
    ...
    return  eta_xz_net_conc, eta_yz_net_conc, eta_xz_error_net_conc, eta_yz_error_net_conc, pearson_xz_net_conc, pearson_yz_net_conc, pearson_xz_error_net_conc, pearson_yz_error_net_conc, \
        eta_xz_net_numbers, eta_yz_net_numbers, eta_xz_error_net_numbers, eta_yz_error_net_numbers, pearson_xz_net_numbers, pearson_yz_net_numbers, pearson_xz_error_net_numbers, pearson_yz_error_net_numbers,\
            lamda, lamda_z, lamda_w, beta_x, beta_y, beta_z, beta_w, k, A, B, C, omega, number_of_simulations

#-------------------------------------------------
#----------------------------------------------

lamdaa = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_lamda.csv')
lamda_wa = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_lamda_w.csv')
lamda_za = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_lamda_z.csv')

beta_xa = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_beta_x.csv')
beta_ya = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_beta_y.csv')
beta_za = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_beta_z.csv')
beta_wa = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_beta_w.csv')
ka = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_k.csv')


AAA = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_A.csv')

BB = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_B.csv')
CC = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_C.csv')
omegaa = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_omega.csv')


number_of_simulations_previous = np.loadtxt(path_2 + '/system_' +system_number +'/vol_dynamic_'+volume_number+'/results/bad_parameters/rerun/bad_parameters/s'+system_number+'_v'+volume_number+'_bad_conc_number_of_simulations.csv')


if lamdaa.ndim > 0:
    blank = np.transpose (np.zeros((len(lamdaa), 11) ) )
    blank[ 0 , : ] = lamdaa
    blank[1 ,  :] = lamda_za
    blank[2 , :  ] = lamda_wa
    blank[ 3 , : ] = beta_xa
    ...
    blank = [lamdaa, lamda_za, lamda_wa, beta_xa, beta_ya, beta_za, beta_wa, ka, AAA, BB, CC, omegaa]
...
eta_xz, eta_yz, eta_xz_error, eta_yz_error, pearson_xz, pearson_yz, pearson_xz_error, pearson_yz_error,\
    eta_xz_numbers, eta_yz_numbers, eta_xz_error_numbers, eta_yz_error_numbers, pearson_xz_numbers, pearson_yz_numbers, pearson_xz_error_numbers, pearson_yz_error_numbers, lamda, lamda_z, lamda_w, beta_x, beta_y, beta_z, beta_w, k, A, B, C, omega, number_of_simulations = new_vol_dyn_6.flatten_3d(result)



#Rereun save path
save_path =  path_2 + '/system_' +system_number+'/vol_dynamic_'+volume_number+ '/results/bad_parameters/rerun/bad_parameters/rerun/rerun_s'+ system_number+'_v'+volume_number
...
np.savetxt(save_path + '_A.csv', A, delimiter = ',')
np.savetxt(save_path + '_B.csv', B, delimiter = ',')
np.savetxt(save_path + '_C.csv', C, delimiter = ',')
np.savetxt(save_path + '_omega.csv', omega, delimiter = ',')
