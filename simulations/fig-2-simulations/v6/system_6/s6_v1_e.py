...
path_2 = '/scratch/h/hilfinge/euanjs/v6'
...
def parallel_s5_v1(inputs):
   ...
   # stay_prob[0: -1 : 2] = 0.6
    ...
    while (AA > 0):
        ...
        if etaxz_conc_err/etaxz_conc_mean < min_err and etayz_conc_err/etayz_conc_mean < min_err and etaxz_num_err/etaxz_num_mean < min_err and etayz_num_err/etayz_num_mean < min_err:
            #pool.close()
            AA = 0
        elif number_of_simulations > 999:
            #pool.close()
            AA = 0
        elif etaxz_conc_err == 0:
            #pool.close()
            AA = 0
...
#-------------------------------------------------
#Standard parameter usage for first time simulation run


lamda_start = 0.1
lamda_end = 10
lamdaa = np.array([0.1,1,10])*1
random.shuffle(lamdaa)      #np.linspace(lamda_start, lamda_end ,5)

lamda_z_start = 0.1
lamda_z_end = 10
lamda_za = np.array([0.1,1,10])
random.shuffle(lamda_za)    #np.linspace(lamda_z_start, lamda_z_end, 2)

lamda_w_start = 0
lamda_w_end = 10
lamda_wa = np.array([0.1,1,10])
random.shuffle(lamda_wa) #np.linspace(lamda_w_start, lamda_w_end, 1)#lamda_w same as lamda_z for corepression

beta_x_start = 0.1
beta_x_end = 10
beta_xa = np.array([0.1,1,10])
random.shuffle(beta_xa) #np.linspace(beta_x_start, beta_x_end, 2)

beta_z_start = 0.1
beta_z_end = 10
beta_za = np.array([0.1,1,10])
random.shuffle(beta_za)     # np.linspace(beta_z_start, beta_z_end,2)

beta_w_start = 0
beta_w_end = 0
beta_wa = np.array([0.1,1,10])
random.shuffle(beta_wa)    #np.linspace(beta_w_start, beta_w_end, 1) #beta_w same as beta_z for corepression

ka = np.array([0.1,1,10])
random.shuffle(ka)

AAA = np.array([0.1,1,10])*1
random.shuffle(AAA)      #np.linspace(lamda_start, lamda_end ,5)

BB = np.array([0.1,1,10])
random.shuffle(BB)    #np.linspace(lamda_z_start, lamda_z_end, 2)

CC = np.array([0.1,1,10])
random.shuffle(CC) #np.linspace(lamda_w_start, lamda_w_end, 1)#lamda_w same as lamda_z for corepression

omegaa = np.array([0.1,1,10])
random.shuffle(omegaa) #np.linspace(lamda_w_start, lamda_w_end, 1)#lamda_w same as lamda_z for corepression


products_long = list(itertools.product(lamdaa, lamda_za, lamda_wa,  beta_xa,  beta_za, beta_wa, ka, AAA, BB, CC, omegaa))
random.shuffle(products_long)
products = products_long[:number_of_parameters]

#----------------------------------------------

#-----------------------------------------------------------
#Parameters to rerun simulation to check points move closer to line
# =============================================================================
# 
# =============================================================================
# lamdaa = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_lamda.csv')
# 
# lamdaa = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_lamda.csv')
# lamda_w = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_lamda_w.csv')
# lamda_z = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_lamda_z.csv')
# 
# beta_x = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_beta_x.csv')
# beta_y = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_beta_y.csv')
# beta_z = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_beta_z.csv')
# beta_w = np.loadtxt(path_2 + '/system_5/vol_dynamic_2/results/bad_parameters/s5_v2_bad_beta_w.csv')
# 
# 
...
pool = multiprocessing.Pool(processes = number_of_cores) #first argument after .py file is number of cores
#result = pool.map(parallel_s5_v1, products) 
#pool.close()
...
for i in range(number_of_parameters):
    ...
    #Rereun save path
    #save_path = save_path = path_2 + '/system_' +system_number+'/vol_dynamic_'+volume_number+ '/results/bad_parameters//rerun/rerun_s'+ system_number+'_v'+volume_number
