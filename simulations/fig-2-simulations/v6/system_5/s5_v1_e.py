...
system_number = '5'  #first argument after .py file is number of cores
volume_number = '1'
...
def s5_v1_covariance(inputs2):
    ...
    volume = new_vol_dyn.exp_V(1, interval_time)
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = 2*lamda * volume / (1 + ( (x/volume) / (0.01 + z/volume))**3 )
            x_death_rate = beta_x * x
            
            y_birth_rate = lamda * volume / (1 + ( (x/volume) / (0.01 + z/volume))**3 )
            y_death_rate = beta_y * y
            
            z_birth_rate = lamda_z * volume
            ...
            maximal_rate =  lamda_z * Vmax + 3 * lamda * Vmax #Set with the assumption that volume at most
...
# =============================================================================
                ...
                interval_time = 1 #+= jump_times[T] - time
...
def parallel_s5_v1(inputs):
    ...
    a2 = np.log(2) #Allows for cell to at most double per interval of 1 second; so that it can be halved after 1 second
    ...
    products2 = [lamda2, lamda_z2, lamda_w2  ,  beta_x2 ,beta_y2, \
            beta_z2,beta_w2,  k2, V02,Vmax2,  a2, 0,0, 1,  1, 1, 1, jump_time_1_factor]
...
products_long = list(itertools.product(lamdaa, lamda_za, lamda_wa,  beta_xa,  beta_za, beta_wa, ka))
