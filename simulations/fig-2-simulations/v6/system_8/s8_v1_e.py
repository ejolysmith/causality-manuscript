...
system_number = '8'  #first argument after .py file is number of cores
volume_number = '1'
...
def s5_v1_covariance(inputs2):
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = lamda / (1 + (   (x/volume) / (0.001 + w)   )**3 )
            x_death_rate = beta_x * x
            
            y_birth_rate = lamda / (1 + (   (x/volume) / (0.001 + w)   )**3 )
            y_death_rate = beta_y * y
            
            z_birth_rate = lamda_z / (1 + (   (w/volume) / (k)   )**3 )
            z_death_rate = beta_z * z
            
            w_birth_rate = lamda_z / (1 + (   (z/volume) / (k)   )**3 )
            w_death_rate = beta_z * w
            
            
            
            maximal_rate =  2 * (lamda_z + lamda)  #Set with the assumption that volume at most
