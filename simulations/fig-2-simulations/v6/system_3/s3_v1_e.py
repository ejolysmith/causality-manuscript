...
system_number = '3'  #first argument after .py file is number of cores
volume_number = '1'
...
def s5_v1_covariance(inputs2):
    ...
    while time < endtime/100:
        ...
        if T < len(jump_times) - 1 : #If we have some jumps remaining
            ...
            x_birth_rate = lamda * w / (k + x ** 2)
            x_death_rate = beta_x * x
            
            y_birth_rate = lamda * w / (k + x ** 2)
            y_death_rate = beta_y * y
            
            z_birth_rate = lamda_z * w
            z_death_rate = beta_z * z
            
            w_birth_rate = lamda_w #System 5 has no w
            w_death_rate = beta_w * w
