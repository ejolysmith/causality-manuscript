...
class xpreader:
    ...
    def __init__(
        ...
        roi_channel: int = 0  #Added by Euan
    ):
        """
        ...
        filenamesindexing = int
            ...
            The default is 1
        segmentation_channel : int, optional
            channel index to use for segmentation, starting at 0. Default is 0.
        ...
        self.roi_channel = roi_channel #Added by Euan
...
def singlecell_features(
    ...
    # Fluo features half added by Euan: #####################
    fluo_features_half = [x for x in features if x[0:4] == "half"]
    if len(fluo_features_half) > 0:
        if fluo_frames is None:
            raise ValueError("You did not provide fluo frames")
        if len(fluo_features_half) != fluo_frames.shape[0]:
            raise ValueError(
                (
                    "You should provide exactly as many fluo frames as you want "
                    "fluo features to extract. Here fluo_frames.shape[0]=%d but "
                ...
                % (fluo_frames.shape[0], len(fluo_features_half), str(fluo_features_half))
            )
        fluo_values_half = cell_fluo_half(fluo_frames, mask, roi_box)
        for fluo_feat, fluo_val in zip(fluo_features_half, fluo_values_half):
            features_dict[fluo_feat] = fluo_val
    ##########################################################################################################
...
def cell_fluo(
    ...
    for frame in range(fluo_frames.shape[0]):
        nonzero_elements = np.asarray(fluo_frames[frame, - 10])
        nonzero_elements_counts = (nonzero_elements > 0).sum()
        stepe = 1
        
        #Removing background fluorescence ----------------------------- Added by Euan 2022
        background_fluorescence = 0
        if len( fluo_frames[frame][max(pixels[0])+15:] ) >= 50 :
            if len( fluo_frames[frame][max(pixels[0])+15:][0][:min(pixels[1])] ) >= 25 and len( fluo_frames[frame][max(pixels[0])+15:][0][min(pixels[1]):] ) >= 25 :
                for stepp in range(50):
        	        background_fluorescence = background_fluorescence + np.sum(fluo_frames[frame][max(pixels[0])+15:max(pixels[0])+15+50][stepp][min(pixels[1])-25 : min(pixels[1])+25 ] )
                background_fluorescence = background_fluorescence/50/50
            else: 
                minwidth = min( [len( fluo_frames[frame][max(pixels[0])+15:][0][:min(pixels[1])] ) , len( fluo_frames[frame][max(pixels[0])+15:][0][min(pixels[1]):]) ] ) #- 1
                for stepp in range(2*minwidth):
                    background_fluorescence = background_fluorescence + np.sum(fluo_frames[frame][max(pixels[0])+15:max(pixels[0])+15+50][stepp][min(pixels[1])-minwidth : min(pixels[1])+minwidth ] )
                if minwidth > 0:
                    background_fluorescence = background_fluorescence/50/2/minwidth
                else: 
                    background_fluorescence = 0
        else:
            if len( fluo_frames[frame][max(pixels[0])+15:][0][:min(pixels[1])] ) >= 25 and len( fluo_frames[frame][max(pixels[0])+15:][0][min(pixels[1]):] ) >= 25 :
                for stepp in range(len( fluo_frames[frame][max(pixels[0])+15:] )):
                    background_fluorescence = background_fluorescence + np.sum(fluo_frames[frame][max(pixels[0])+15:][stepp][min(pixels[1])-25 : min(pixels[1])+25 ] )
                background_fluorescence = background_fluorescence/len( fluo_frames[frame][max(pixels[0])+15:] )/50
            else: 
                minwidth = min( [len( fluo_frames[frame][max(pixels[0])+15:][0][:min(pixels[1])] ) , len( fluo_frames[frame][max(pixels[0])+15:][0][min(pixels[1]):]) ] ) #- 1
                for stepp in range(2*minwidth):
                    background_fluorescence = background_fluorescence + np.sum(fluo_frames[frame][max(pixels[0])+15:][stepp][min(pixels[1])-minwidth : min(pixels[1])+minwidth ] )
                background_fluorescence = background_fluorescence/len( fluo_frames[frame][max(pixels[0])+15:] )/2/minwidth
        		
        fluo_values += [np.mean(fluo_frames[frame, pixels[0], pixels[1]]) - background_fluorescence]
        #---------------------------------------------------------------------------------------------------------------
    return fluo_values
...
def cell_fluo_half(
    ...
    for frame in range(fluo_frames.shape[0]):
        ...
        pixels_0_half = []
        pixels_1_half = []
        for _ in range(len(pixels[0])):
            for __ in range(len(pixels[1])):
                random_number = random.random()
                if random_number < 0.5:
                    pixels_0_half.append(pixels[0][_])
                    pixels_1_half.append(pixels[1][__])
        
        pixels_0_half = np.array(pixels_0_half)
        pixels_1_half = np.array(pixels_1_half)
        
        fluo_values_half += [np.mean(fluo_frames[frame, pixels_0_half, pixels_1_half]) - background_fluorescence]
        #---------------------------------------------------------------------------------------------------------------
    return fluo_values_half
