...
class Pipeline:
    ...
    def process(
        ...
        if features is None:
            ...
            for c in range(0, self.reader.channels):   #<<<<< EJ: Was range(1, self.reader.channels) before
                features_list += ["fluo%d" % (c,)]
            for c in range(0, self.reader.channels):   #<<<<< EJ: added for half fluorescence
                features_list += ["half%d" % (c,)]
...
class Position:
    ...
    def preprocess(
        ...
        if reference is None:
            reference = self.reader.getframes(
                ...
                frames=0,
                channels=self.reader.roi_channel, #self.reader.segmentation_channel, #####################Changed
                rescale=(0, 1),
    ...
    def detect_rois(self, reference: utils.Image):
        ...
        """
        from delta.extra import getROIBoxes_target_size

        # cv2.resize expect x,y
        # Predict
        roismask = self.models["rois"].predict(
            utils.rangescale(cv2.resize(reference, cfg.target_size_rois[::-1]), (0, 1))[
        ...
        roisboxes = getROIBoxes_target_size(roismask, cfg.target_size_seg)
        # Instanciate ROIs:
    ...
    def segment(self, frames: List[int]):
        ...
        trans_frames = self.reader.getframes(
            positions=self.position_nb,
            channels=self.reader.segmentation_channel,
            frames=frames,
            rescale=(0, 1),
            rotate=self.rotate,
        )
