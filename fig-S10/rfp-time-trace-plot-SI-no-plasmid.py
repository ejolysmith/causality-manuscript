...
for p in range(0, 0 + number_of_positions):
    ...
    for cell in range(0,len(final_cfp_load)):    
        if len(final_lengths_load[cell]) > 0 :
            ...
            if len(division_times) > 0:
                ...
                generation_time = 6.103*5
                divisions_cells_p.append(np.array(division_times)*5/generation_time)
    ...
    if len(pixels_cells_p) > 0 : 
        ...
        for k in range(len(unique_cells)):
            ...
            for kk in [0]:#range(len(where_cells_is)):
                ...
                for jjj in range(len(divisions_cells_p[cell_index])):
                    division_times_temp.append(divisions_cells_p[cell_index][jjj])
...
ax.plot(t1[A:len(growth_rate_smooth_cells[CELL])] - t1[A], 1*rfp_smooth_cells[CELL][A:len(growth_rate_smooth_cells[CELL])]/2100, color = 'r')
...
ax1.set_ylim([0.45,2.1])
ax.set_ylim([0.35,2.1])
ax1.set_xlim([-1,21])
ax.set_xlim([-1,21])
ax.plot(divisions_cells[CELL], np.ones(len(divisions_cells[CELL]))*2, '.')

ax1.set_ylim([0.35,2.1])
ax1.set_yticks(ticks=[0.5, 1, 1.5])#, labels=['0.5', '1.0', '1.5'])
ax.set_yticks(ticks=[0.5, 1, 1.5])#, labels=['0.5', '1.0', '1.5'])
ax.set_xticks(ticks=[0, 5, 10,15,20])#, labels=['0.5', '1.0', '1.5'])
