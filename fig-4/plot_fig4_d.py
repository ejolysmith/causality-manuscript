        ...
        for k in range(len(unique_cells)):
            ...
            for kk in range(len(where_cells_is)):
                ...
                for jjj in range(len(  growth_rate_smooth_cells_p[cell_index]  ) ):
                    gfp_temp.append(gfp_cells_p[cell_index][jjj])
...
t1 = np.array(range(len(gfp_smooth_cells[CELL])))*5./generation_time
t2 = np.array(range(len(rfp_smooth_cells[CELL])))*5./generation_time
ax.plot(t2 , rfp_smooth_cells[CELL]/np.average(rfp_smooth_cells[CELL]), color = 'darkred')
ax.set_xlabel('Time (generations)')
ax.set_ylabel('Concentration (a.u.)')

ax1 = ax.twinx()
ax1.plot( t2  ,gfp_smooth_cells[CELL]/np.average(gfp_smooth_cells[CELL]), '--', color = '0.5')
