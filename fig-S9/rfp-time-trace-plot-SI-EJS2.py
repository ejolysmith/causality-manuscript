...
for p in range(12, 12 + number_of_positions):
    ...
    for cell in range(0,len(final_cfp_load)):    
        if len(final_lengths_load[cell]) > 0 :
            ...
            for i in range(len(final_lengths_load[cell])-1):
                if final_lengths_load[cell][i+1]  < 0.8*final_lengths_load[cell][i]:    
    ...
    if len(pixels_cells_p) > 0 : 
        ...
        for k in range(len(unique_cells)):
            ...
            for _ in range(len(pixels_cells_p_array)):
                if pixels_cells_p_array[_] == unique_cells[k]:
                    where_cells_is = _
                    break
...
CELL = 68 #68 82 84 86 90 102 104
A = 0
...
ax.plot(t1[A:len(growth_rate_smooth_cells[CELL])] - t1[A], 1*rfp_smooth_cells[CELL][A:len(growth_rate_smooth_cells[CELL])]/4100, color = 'r')
...
ax1.set_yticks(ticks=[0.5, 1, 1.5])
