...
for p in range(0, 0 + number_of_positions):
    ...
    with open('../data/delta/EJS9_repeat/yfp_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_yfp_load = pickle.load(f)
        
    with open('../data/delta/EJS9_repeat/cfp_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_cfp_load = pickle.load(f)
    
    with open('../data/delta/EJS9_repeat/rfp_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_rfp_load = pickle.load(f)
        
    with open('../data/delta/EJS9_repeat/lengths_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_lengths_load = pickle.load(f)
        
    with open('../data/delta/EJS9_repeat/areas_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_areas_load = pickle.load(f)
        
    with open('../data/delta/EJS9_repeat/frames_purged_pos' +str(p) + '-purge2.pkl', 'rb') as f:
        final_frames_load = pickle.load(f)
        
    with open('../data/delta/EJS9_repeat/frames_purged_xpixels' +str(p) + '-purge2.pkl', 'rb') as f:
    ...
    for cell in range(0,len(final_cfp_load)):    
        if len(final_lengths_load[cell]) > 0 :
            ...
            if len(division_times) > 0:
                ...
                divisions_cells_p.append(np.array(division_times)*5/32.433)
    ...
    if len(pixels_cells_p) > 0 : 
        ...
        for k in range(len(unique_cells)):
            ...
            for kk in range(len(where_cells_is)):
                ...
                for kkk in range(len(divisions_cells_p[cell_index])):
                    division_times_temp.append(divisions_cells_p[cell_index][kkk])
            ...
            divisions_cells.append(division_times_temp)
...
t1 = np.array(range(len(rfp_smooth_cells[CELL])))*5./generation_time
t2 = np.array(range(len(growth_rate_smooth_cells[CELL])))*5./generation_time
ax.plot(t1[A:] - t1[A], 1*rfp_smooth_cells[CELL][A:]/1500, color = 'r')

ax.set_xlabel('Time (generations)')
ax.set_ylabel('RFP concentration (a.u.)')
ax1 = ax.twinx()
ax1.plot( t2[A:] - t2[A] , growth_rate_smooth_cells[CELL][A:])
ax1.set_ylabel('Growth rate (1/h)', rotation=270,labelpad=15)
ax.plot(divisions_cells[CELL], np.ones(len(divisions_cells[CELL]))*1.6, '.')
plt.tight_layout()
