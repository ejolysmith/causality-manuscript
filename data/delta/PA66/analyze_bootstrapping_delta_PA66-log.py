    ...
    if len(pixels_cells_p) > 0 : 
        ...
        for k in range(len(unique_cells)):
            ...
            for kk in range(len(where_cells_is)):
                ...
                for kkk in range(len(  cfp_cells_p[cell_index]  ) ):
                    cfp_temp.append(cfp_cells_p[cell_index][kkk])
...
for p in range( 22, 22 + number_of_positions ):
    ...
    with open('yfp_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_yfp_load = pickle.load(f)
        
    with open('cfp_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_cfp_load = pickle.load(f)
    
    with open('rfp_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_rfp_load = pickle.load(f)
        
    with open('lengths_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_lengths_load = pickle.load(f)
        
    with open('areas_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_areas_load = pickle.load(f)
        
    with open('frames_purged_pos' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_frames_load = pickle.load(f)
        
    with open('frames_purged_xpixels' +str(p) + '-purge2-b.pkl', 'rb') as f:
        final_xpixels_load = pickle.load(f)
...
for sample in range(number_of_samples):
    ...
    for cell in range(number_of_cells):
        ...
        for k in range(len(frames_cells[random_cell])):
            ...
            areas_sample.append(areas_cells[random_cell][k])
    ...
    areas_sample = np.array(areas_sample)
    ...
    for cell in range(number_of_cells_b):
        ...
        for k in range(len(frames_cells_b[random_cell_b])):
            ...
            areas_sample_b.append(areas_cells_b[random_cell_b][k])
    ...
    areas_sample_b = np.array(areas_sample_b)
    ...
    for i in range(len(rfp_sample_b)):
        rfp_sample_b[i] = rfp_sample_b[i]/P(int(frames_sample_b[i]))*P(0)
    ...
    for i in range(len(yfp_sample_b)):
        yfp_sample_b[i] = yfp_sample_b[i]/P(int(frames_sample_b[i]))*P(0)
    ...
    for i in range(len(cfp_sample_b)):
        cfp_sample_b[i] = cfp_sample_b[i]/P(int(frames_sample_b[i]))*P(0)
...
###   Getting autofluorescence    ###
    ...
    for i in range(len(pixels_sample_b)):
        ...
        if frames_sample_b[i] > min_frame and frames_sample_b[i] < max_frame:
            ...
            areas_average_b = areas_average_b + areas_sample_b[i]
            
            xx_bins_b = xx_bins_b + cfp_sample_b[i]*cfp_sample_b[i]/P_cfp(pixels_sample_b[i])*z_cfp_max/P_cfp(pixels_sample_b[i])*z_cfp_max
            yy_bins_b = yy_bins_b + yfp_sample_b[i]*yfp_sample_b[i]/P_yfp(pixels_sample_b[i])*z_yfp_max/P_yfp(pixels_sample_b[i])*z_yfp_max

            xz_bins_b = xz_bins_b + cfp_sample_b[i]*rfp_sample_b[i]/P_rfp(pixels_sample_b[i])*z_rfp_max/P_cfp(pixels_sample_b[i])*z_cfp_max
            yz_bins_b = yz_bins_b + yfp_sample_b[i]*rfp_sample_b[i]/P_rfp(pixels_sample_b[i])*z_rfp_max/P_yfp(pixels_sample_b[i])*z_yfp_max

            xg_bins_b = xg_bins_b + cfp_sample_b[i]*growth_rate_sample_b[i]/P_cfp(pixels_sample_b[i])*z_cfp_max
            yg_bins_b = yg_bins_b + yfp_sample_b[i]*growth_rate_sample_b[i]/P_yfp(pixels_sample_b[i])*z_yfp_max

            xa_bins_b = xa_bins_b + cfp_sample_b[i]*areas_sample_b[i]/P_cfp(pixels_sample_b[i])*z_cfp_max
            ya_bins_b = ya_bins_b + yfp_sample_b[i]*areas_sample_b[i]/P_yfp(pixels_sample_b[i])*z_yfp_max
    ...
    if number_b > 0 :
        ...
        areas_average_b = areas_average_b/number_b
        ...
        xa_bins_b = xa_bins_b/number_b
        ya_bins_b = ya_bins_b/number_b  
        
        varx_b = xx_bins_b - cfp_average_b**2
        vary_b = yy_bins_b - yfp_average_b**2

        covxz_b = xz_bins_b - cfp_average_b*rfp_average_b
        covyz_b = yz_bins_b - yfp_average_b*rfp_average_b
        
        covxg_b = xg_bins_b - cfp_average_b*growth_rate_average_b
        covyg_b = yg_bins_b - yfp_average_b*growth_rate_average_b
        
        covxa_b = xa_bins_b - cfp_average_b*areas_average_b
        covya_b = ya_bins_b - yfp_average_b*areas_average_b
...
### CALCULATING ETAS ###
        ...
        for i in range(len(pixels_sample)):
            ...
            if frames_sample[i] > min_frame and frames_sample[i] < max_frame:
                ...
                areas_average = areas_average + areas_sample[i]
                
                xz_bins = xz_bins + cfp_sample[i]*rfp_sample[i]/P_rfp(pixels_sample[i])*z_rfp_max/P_cfp(pixels_sample[i])*z_cfp_max
                yz_bins = yz_bins + yfp_sample[i]*rfp_sample[i]/P_rfp(pixels_sample[i])*z_rfp_max/P_yfp(pixels_sample[i])*z_yfp_max
                        
                xg_bins = xg_bins + growth_rate_sample[i]*cfp_sample[i]/P_cfp(pixels_sample[i])*z_cfp_max
                yg_bins = yg_bins + growth_rate_sample[i]*yfp_sample[i]/P_yfp(pixels_sample[i])*z_yfp_max
                
                xa_bins = xa_bins + areas_sample[i]*cfp_sample[i]/P_cfp(pixels_sample[i])*z_cfp_max
                ya_bins = ya_bins + areas_sample[i]*yfp_sample[i]/P_yfp(pixels_sample[i])*z_yfp_max
    
                xx_bins = xx_bins + cfp_sample[i]*cfp_sample[i]/P_cfp(pixels_sample[i])*z_cfp_max/P_cfp(pixels_sample[i])*z_cfp_max
                ...
                aa_bins = aa_bins + areas_sample[i]*areas_sample[i]
        ...
        if number > 0 :
            ...
            areas_average = areas_average/number
            ...
            aa_bins = aa_bins/number
                
            xy_bins = xy_bins/number
            
            covxz = (xz_bins - cfp_average*rfp_average)
            covyz = (yz_bins - yfp_average*rfp_average)
            
            covxg = (xg_bins - cfp_average*growth_rate_average)
            covyg = (yg_bins - yfp_average*growth_rate_average)
            covxa = (xa_bins - cfp_average*areas_average)
            covya = (ya_bins - yfp_average*areas_average)
            
            varx = (xx_bins - cfp_average**2)
            vary = (yy_bins - yfp_average**2)
            varz = (zz_bins - rfp_average**2)
            
            varg = (gg_bins - growth_rate_average**2)
            vara = (aa_bins - areas_average**2)
            ...
            #Final etas:
            etaxz = covxz_2/cfp_average_2/rfp_average
            etayz = covyz_2/yfp_average_2/rfp_average
            
            etaxg = covxg_2/cfp_average_2/growth_rate_average
            etayg = covyg_2/yfp_average_2/growth_rate_average
            etaxa = covxa_2/cfp_average_2/areas_average
            etaya = covya_2/yfp_average_2/areas_average
            ...
            rhoxa = covxa_2/np.sqrt(varx_2*vara)
            rhoya = covya_2/np.sqrt(vary_2*vara)
            ...
            etaxa_final.append(etaxa)
            etaya_final.append(etaya)
            rhoxa_final.append(rhoxa)
            rhoya_final.append(rhoya)
            ...
            abar_final.append(areas_average)
...
final_data = {'etaxz': ETAXZ, 'etaxz_err': ETAXZ_ERR, 'etayz': ETAYZ, 'etayz_err': ETAYZ_ERR, \
              ...
              'rfp_avg': ZBAR, 'rfp_avg_err': ZBAR_ERR,  \
              'CVz': CVZ, 'CVz_err': CVZ_ERR, 'CVg': CVG, 'CVg_err': CVG_ERR}
    
    
with open('../final_statistics/PA66_bootstrapping.pkl', 'wb') as f:
