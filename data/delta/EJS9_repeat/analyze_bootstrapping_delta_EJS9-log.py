    ...
    for cell in range(0,len(final_cfp_load)):    
        if len(final_lengths_load[cell]) > 0 :
            ...
            for i in range(len(final_lengths_load[cell])-1):
                if final_lengths_load[cell][i+1]  < 0.7*final_lengths_load[cell][i]:    #This finds the divisions
            ...
            if len(division_times) > 0:
                if division_times[0] > 1 :
                    temp_growth_rates = []
                    growth_rate  =  ( division_times[1]/division_times[0] )/time_to_divide[0]
...
for sample in range(number_of_samples):
    ...
    for i in range(len(frames_sample)):
        if frames_sample[i] < 55 or frames_sample[i] > 70:
    ...
    for i in range(len(cfp_time_bins_num)):
        if cfp_time_bins_num[i] > 0:
            cfp_time_bins[i] = cfp_time_bins[i]/cfp_time_bins_num[i]
            time_bins.append(i)
    cfp_time_bins = cfp_time_bins[cfp_time_bins != 0]
    ...
    for i in range(len(yfp_time_bins_num)):
        if yfp_time_bins_num[i] > 0:
            yfp_time_bins[i] = yfp_time_bins[i]/yfp_time_bins_num[i]
            #time_bins.append(i)
    yfp_time_bins = yfp_time_bins[yfp_time_bins != 0]
    ...
    for i in range(len(rfp_time_bins_num)):
        if rfp_time_bins_num[i] > 0:
            rfp_time_bins[i] = rfp_time_bins[i]/rfp_time_bins_num[i]
            #time_bins.append(i)
    rfp_time_bins = rfp_time_bins[rfp_time_bins != 0]


    
    #In the following, 50 is set at the initial time trace cut off due to photobleaching.
    #Correcting the time-drift:
    Z = np.polyfit(np.array(time_bins[50:]), rfp_time_bins[50:], 3)
    P = np.poly1d(Z)
    ...
    Z = np.polyfit(np.array(time_bins[50:]), yfp_time_bins[50:], 2)
    P = np.poly1d(Z)
    ...
    Z = np.polyfit(np.array(time_bins[50:]), cfp_time_bins[50:], 2)
    P = np.poly1d(Z)
...
final_data = {'etaxz': ETAYZ, 'etaxz_err': ETAYZ_ERR, 'etayz': ETAXZ, 'etayz_err': ETAXZ_ERR, \
              'etaxg': ETAYG, 'etaxg_err': ETAYG_ERR, 'etayg': ETAXG, 'etayg_err': ETAXG_ERR, \
              'etaxa': ETAYL, 'etaxa_err': ETAYL_ERR, 'etaya': ETAXL, 'etaya_err': ETAXL_ERR, \
              'rhoxz': RHOYZ, 'rhoxz_err': RHOYZ_ERR, 'rhoyz': RHOXZ, 'rhoyz_err': RHOXZ_ERR, \
              'rhoxg': RHOYG, 'rhoxg_err': RHOYG_ERR, 'rhoyg': RHOXG, 'rhoyg_err': RHOXG_ERR, \
              'rhoxl': RHOYL, 'rhoxl_err': RHOYL_ERR, 'rhoyl': RHOXL, 'rhoya_err': RHOXL_ERR, \
              'xavg' : YBAR,  'xavg_err' : YBAR_ERR,  'yavg' : XBAR,  'yavg_err' : XBAR_ERR,  \
              'l_avg': LBAR,  'l_avg_err': LBAR_ERR,  'growth_rate_avg': GBAR, 'growth_rate_avg_err': GBAR_ERR, \
              'generation_time': GENERATION_TIME}
    
    
    
with open('../final_statistics/EJS9_repeat_bootstrapping.pkl', 'wb') as f:
