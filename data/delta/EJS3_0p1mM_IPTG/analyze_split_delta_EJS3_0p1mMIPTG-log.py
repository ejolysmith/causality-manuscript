...
for sample in range(number_of_samples):
    ...
    for i in range(len(frames_sample)):
        if frames_sample[i] < 55 or frames_sample[i] > 70:   #Purging measurement noise
...
ETAXL_ERR = np.std(etaxl_final)*2/np.sqrt(number_of_samples-1)
ETAYL = np.average(etayl_final)
ETAYL_ERR = np.std(etayl_final)*2/np.sqrt(number_of_samples-1)
...
final_data = {'etaxz': ETAXZ, 'etaxz_err': ETAXZ_ERR, 'etayz': ETAYZ, 'etayz_err': ETAYZ_ERR, \
              ...
              'a_avg': LBAR,  'a_avg_err': LBAR_ERR,  'growth_rate_avg': GBAR, 'growth_rate_avg_err': GBAR_ERR, \
              'generation_time': GENERATION_TIME}
    
    
with open('final_statistics/EJS3_0p1mMIPTG_split.pkl', 'wb') as f:
