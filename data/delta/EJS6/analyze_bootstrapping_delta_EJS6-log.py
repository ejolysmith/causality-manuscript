...
for i in range(number_of_cells):
    for j in range(len(cfp_cells[i])):
        cfp_cells[i][j] = cfp_cells[i][j] #- min_cfp_b 
        yfp_cells[i][j] = yfp_cells[i][j] #- min_yfp_b

        
for i in range(number_of_cells_b):
    for j in range(len(cfp_cells_b[i])):
        cfp_cells_b[i][j] = cfp_cells_b[i][j] #- min_cfp_b
        yfp_cells_b[i][j] = yfp_cells_b[i][j] #- min_yfp_b
...
for sample in range(number_of_samples):
    ...
    for i in range(len(frames_sample)):
        if frames_sample[i] < 100 or frames_sample[i] > 135: #purging measurement noise
    ...
    for i in range(len(frames_sample)):
        if frames_sample[i] < 100 or frames_sample[i] > 135:
            if frames_sample[i] < number_of_frames:
...
with open('final_statistics/EJS6_bootstrapping.pkl', 'wb') as f:
