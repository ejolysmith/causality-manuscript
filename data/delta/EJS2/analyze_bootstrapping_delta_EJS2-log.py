...
for p in range(0, 19 ):
    ...
    #Backgrounds were taken from EJS cells that lost plasmid. 
    #This is becaue EJS1 and EJS2 were done in same experiment (different lanes of same mother machine chip).
    #There were too few EJS2 cells that lost plasmid to use to quantify background. 
    with open('../EJS1/yfp_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_yfp_load = pickle.load(f)
        
    with open('../EJS1/cfp_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_cfp_load = pickle.load(f)
    
    with open('../EJS1/rfp_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_rfp_load = pickle.load(f)
        
    with open('../EJS1/lengths_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_lengths_load = pickle.load(f)
        
    with open('../EJS1/areas_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_areas_load = pickle.load(f)
        
    with open('../EJS1/frames_purged_pos' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_frames_load = pickle.load(f)
        
    with open('../EJS1/frames_purged_xpixels' +str(p) + '-purge2_b.pkl', 'rb') as f:
        final_xpixels_load = pickle.load(f)            
        ...
        for j in range(len(pixels_sample)):
            if i*bsize <= pixels_sample[j] < (i+1)*bsize:
                if frames_sample[j] > min_frame and frames_sample[j] < max_frame:
                    cfp_average = cfp_average + cfp_sample[j]#/P_cfp((i+0.5)*bsize)*z_cfp_max
...
###   Getting autofluorescence    ###
    ...
    for i in range(len(pixels_sample_b)):
        ...
        if frames_sample_b[i] > min_frame and frames_sample_b[i] < max_frame:
            ...
            gg_bins_b = gg_bins_b + growth_rate_sample_b[i]*growth_rate_sample_b[i]
            number_b = number_b + 1
    ...
    if number_b > 0 :
        ...
        varg_b = gg_bins_b - growth_rate_average_b**2
        
        cvg_b = np.sqrt(varg_b)/growth_rate_average_b
...
### CALCULATING ETAS ###
        ...
        if number > 0 :
            ...
            cvx = np.sqrt(varx/yfp_average_2/yfp_average_2)
...
CVX = np.average(cvx_final)
CVX_ERR = np.std(cvx_final)*2
...
with open('final_statistics/EJS2_normalMC_bootstrapping.pkl', 'wb') as f:
